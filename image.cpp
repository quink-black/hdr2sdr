...
std::shared_ptr<Image<float>> ImageWrapper::GetImg() {
    ...
    if (mImgFloat) {
        ...
        img = std::make_shared<Image<float>>(mImgUInt8->mWidth, mImgUInt8->mHeight);
        int n = img->DataLength();
        for (int i = 0; i < n; i++)
            img->mData[i] = mImgUInt8->mData[i] / 255.0f;
    } else if (mImgUInt16) {
        img = std::make_shared<Image<float>>(mImgUInt16->mWidth, mImgUInt16->mHeight);
        int n = img->DataLength();
        for (int i = 0; i < n; i++)
...
std::shared_ptr<Image<uint8_t>> ImageWrapper::GetImg() {
    ...
    if (mImgFloat) {
        img = std::make_shared<Image<uint8_t>>(mImgFloat->mWidth, mImgFloat->mHeight);
        int n = img->DataLength();
        for (int i = 0; i < n; i++)
        ...
        img = std::make_shared<Image<uint8_t>>(mImgUInt16->mWidth, mImgUInt16->mHeight);
        int n = img->DataLength();
        for (int i = 0; i < n; i++)
...
std::shared_ptr<Image<uint16_t>> ImageWrapper::GetImg() {
    ...
    if (mImgFloat) {
        img = std::make_shared<Image<uint16_t>>(mImgFloat->mWidth, mImgFloat->mHeight);
        int n = img->DataLength();
        for (int i = 0; i < n; i++)
            img->mData[i] = std::min<float>(mImgFloat->mData[i] * 255, 65535);
    } else if (mImgUInt8) {
        img = std::make_shared<Image<uint16_t>>(mImgUInt8->mWidth, mImgUInt8->mHeight);
        int n = img->DataLength();
        for (int i = 0; i < n; i++)
